...
export class FeaturesService {
  ...
  addLaptop(laptop: any): Observable<any> {
    ...
    return this.http
      .post<any>(`${this.baseUrl}`, laptop, options)
      .pipe(retry(3), catchError(this.handleError));
  }

  getAllLaptop(pageNo: number, pageSize: number, keyword: string): Observable<any> {
    const token = sessionStorage.getItem("auth_token");
    const options = {
      headers: new HttpHeaders({
        "Content-Type": "application/json",
    ...
    return this.http
      .get<any>(`${this.baseUrl}?page=${pageNo}&size=${pageSize}&keyword=${keyword}`, options)
